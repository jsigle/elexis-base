...
public class FtpServer extends FTPClient {
	...
	public void removeSemaphore() throws IOException{
		...
		if (res) {
			log.log(String.format("Deleted semaphore %s on %s", name, serverAddress), //$NON-NLS-1$
				Log.INFOS);
		} else {
			log.log(String.format("Unable to delete semaphore %s on %s", name, serverAddress), //$NON-NLS-1$
				Log.ERRORS);
			SWTHelper.showError("FTP Semaphore delete error",
