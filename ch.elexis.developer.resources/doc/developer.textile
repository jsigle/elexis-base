...
* the file logback.xml provided via the ch.elexis.logback feature project. This ensures that we have a good default configuration. Before Elexis version 2.1.7.1 logback.xml was specified in the elexis.ini which caused problems in the following situations
** network installation which caused one of the following situations
** the installation directory was renamed afterwards 
** the logback.xml specified in the elexis.ini did not exist (anymore)
* It can be overridden by placing a line like  @-Dlogback.configurationFile=/path/to/my/logback.xml@. One must specify an absolute path, as variables like @user.home will not be expanded.
* @ch.elexis.logback/rsc/logback.xml@ defines the following defaults: 
...
If you want to set the logging of a single logger to another level, edit your logback.xml and add a line like @<logger name="TextContainer" level="WARN"/>@.
...
There you have to add in your run/debug configuration to the under Arguments..Programm Arguments the lines

bq. -osgi.bundles=org.eclipse.core.runtime@start\
,org.eclipse.update.configurator@3:start\
,org.eclipse.equinox.common@2:start\
,ch.qos.logback.classic@2:start\
,ch.qos.logback.core@2:start\
,ch.qos.logback.slf4j@2\
,org.slf4j.api@2:start\
,org.slf4j.jul@2\
,org.eclipse.osgi.services@2:start\
,org.ekkescorner.logging.osgi@2:start\
,LogBackend@3:start

and under Arguments..VM Arguments the line @-Dlogback.configurationFile=/path/to/logback.xml@

!images/logback_configuration.png!

h3. Implementation notes

* The plugin *LogbackBundle* collects *slf4j* log messages and passes them to *ch.qos.logback*.
* The plugin *org.ekkescorner.logging.osgi* 
** redirects standard out and standard error to the logging framework unless @-console@ or @-consoleLog@ are passed as vmargs paramenters. It uses the "*sysout-over-slf4j*":http://projects.lidalia.org.uk/sysout-over-slf4j/faq.html module.
** ollects OSGi messages, e.g. when bundles are started and logs them via slf4j. 
** Its activator also calls @SLF4JBridgeHandler.install()@ to redirect Java.util.logging.
