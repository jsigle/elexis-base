...
public class ImportFileTest {
	...
	void runOneOddbImport(String fileName){
		...
		System.out.println(String.format("Elapsed %1$d.%2$d seconds importing %3$s",
			difference / 1000, difference % 1000, file.getAbsolutePath()));
	}
	...
	void runOneImport(String fileName, boolean useAll){
		...
		if (useAll) {
			...
			System.out.println(fileName + ": size counter is : " + counter + " file was "
				+ file.length());
			...
			System.out.println(fileName + ": size of object is : " + object.toString().length()
				+ " file was " + file.length());
