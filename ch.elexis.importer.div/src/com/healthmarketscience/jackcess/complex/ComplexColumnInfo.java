...
public abstract class ComplexColumnInfo<V extends ComplexValue> {
	...
	protected ComplexColumnInfo(Column column, int complexTypeId, Table typeObjTable,
		Table flatTable) throws IOException{
		_column = column;
		...
		if ((pkCol == null) || (complexValFkCol == null)) {
			throw new IOException("Could not find expected columns in flat table "
				+ flatTable.getName() + " for complex column with id " + complexTypeId);
		}
		_pkCol = pkCol;
	...
	public static ComplexColumnInfo<? extends ComplexValue> create(Column column,
		ByteBuffer buffer, int offset) throws IOException{
		...
		IndexCursor cursor =
			IndexCursor.createCursor(complexColumns, complexColumns.getPrimaryKeyIndex());
		if (!cursor.findFirstRowByEntry(complexTypeId)) {
			throw new IOException("Could not find complex column info for complex column with id "
				+ complexTypeId);
		}
		...
		if (tableId != column.getTable().getTableDefPageNumber()) {
			throw new IOException("Found complex column for table " + tableId
				+ " but expected table " + column.getTable().getTableDefPageNumber());
		}
		...
		if ((typeObjTable == null) || (flatTable == null)) {
			throw new IOException("Could not find supporting tables (" + typeObjId + ", "
				+ flatTableId + ") for complex column with id " + complexTypeId);
		}
	...
	public int countValues(int complexValueFk) throws IOException{
		return getRawValues(complexValueFk, Collections.singleton(_complexValFkCol.getName()))
			.size();
	}
	...
	private Iterator<Map<String, Object>> getComplexValFkIter(int complexValueFk,
		Collection<String> columnNames) throws IOException{
		if (_complexValIdCursor == null) {
			_complexValIdCursor =
				new CursorBuilder(_flatTable).setIndexByColumns(_complexValFkCol).toIndexCursor();
		}
	...
	protected List<V> toValues(ComplexValueForeignKey complexValueFk,
		List<Map<String, Object>> rawValues) throws IOException{
	...
	protected static void diffFlatColumns(Table typeObjTable, Table flatTable,
		List<Column> typeCols, List<Column> otherCols){
	...
	protected abstract V toValue(ComplexValueForeignKey complexValueFk,
		Map<String, Object> rawValues) throws IOException;
	...
	protected static abstract class ComplexValueImpl implements ComplexValue {
		...
		public boolean equals(Object o){
			return ((this == o) || ((o != null) && (getClass() == o.getClass())
				&& (_id == ((ComplexValueImpl) o)._id) && _complexValueFk
					.equals(((ComplexValueImpl) o)._complexValueFk)));
		}
	}
