...
public class MultiValueColumnInfo extends ComplexColumnInfo<SingleValue> {
	private static final Set<DataType> VALUE_TYPES = EnumSet.of(DataType.BYTE, DataType.INT,
		DataType.LONG, DataType.FLOAT, DataType.DOUBLE, DataType.GUID, DataType.NUMERIC,
		DataType.TEXT);
	
	private final Column _valueCol;
	
	public MultiValueColumnInfo(Column column, int complexId, Table typeObjTable, Table flatTable)
		throws IOException{
	...
	@Override
	protected SingleValueImpl toValue(ComplexValueForeignKey complexValueFk,
	...
	public static boolean isMultiValueColumn(Table typeObjTable){
		...
		return ((typeCols.size() == 1) && VALUE_TYPES.contains(typeCols.get(0).getType()));
	}
