...
public enum DataType {
	...
	/**
	 * Corresponds to a java Boolean. Accepts Boolean or {@code null} (which is considered
	 * {@code false}). Equivalent to SQL {@link Types#BOOLEAN}.
	 */
	 ...
	 */
	OLE((byte) 0x0B, Types.LONGVARBINARY, null, true, true, 0, null, 0x3FFFFFFF, 1),
	/**
	...
	// for some reason numeric is "var len" even though it has a fixed size...
		...
		/**
		 * Complex type corresponds to a special LONG autonumber field which is the key for a
		 * secondary table which holds the "real" data.
		 */
		COMPLEX_TYPE((byte) 0x12, null, 4),
		/**
		 * Dummy type for a fixed length type which is not currently supported. Handled like a fixed
		 * length BINARY.
		 */
		UNSUPPORTED_FIXEDLEN((byte) 0xFE, null, null),
		/**
		 * Placeholder type for a variable length type which is not currently supported. Handled
		 * like BINARY.
		 */
		UNSUPPORTED_VARLEN((byte) 0xFF, null, null, true, false, 0, null, 0x3FFFFFFF, 1);
