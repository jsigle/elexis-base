...
public class PropertyMaps implements Iterable<PropertyMap> {
	...
	static final class Handler {
		...
		/**
		 * @return a PropertyMaps instance decoded from the given bytes (always returns non-
		 *         {@code null} result).
		 */
		...
		/**
		 * @return the PropertyMap created from the values parsed from the given data chunk combined
		 *         with the given property names
		 */
		private PropertyMap readPropertyValues(ByteBuffer bbBlock, List<String> propNames,
			short blockType) throws IOException{
		...
		private boolean isPseudoGuidColumn(DataType dataType, String propName, int dataSize){
			...
			return ((dataType == DataType.BINARY) && (dataSize == DataType.GUID.getFixedSize()) && PropertyMap.GUID_PROP
		...
		/**
		 * Normal boolean columns do not write into the actual row data, so we need to do a little
		 * extra work.
		 */
