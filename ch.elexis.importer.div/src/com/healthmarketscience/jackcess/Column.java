...
public class Column implements Comparable<Column> {
	...
	/**
	 * Mask to apply the long length in order to get the flag bits (only the first 2 bits are type
	 * flags).
	 */
	 ...
	 */
	public static final SortOrder GENERAL_LEGACY_SORT_ORDER = new SortOrder(
		GENERAL_SORT_ORDER_VALUE, (byte) 0);
	 ...
	 * @usage _intermediate_field_
	 */
	public static final SortOrder GENERAL_SORT_ORDER = new SortOrder(GENERAL_SORT_ORDER_VALUE,
		(byte) 1);
	...
	public Column(Table table, ByteBuffer buffer, int offset, int displayIndex) throws IOException{
		...
		if (_type.getHasScalePrecision()) {
			...
			_textInfo._sortOrder =
				readSortOrder(buffer, offset + getFormat().OFFSET_COLUMN_SORT_ORDER, getFormat());
			int cpOffset = getFormat().OFFSET_COLUMN_CODE_PAGE;
			if (cpOffset >= 0) {
				_textInfo._codePage = buffer.getShort(offset + cpOffset);
			}
			
			_textInfo._compressedUnicode =
	...
	/**
	 * Whether or not this column is "append only" (its history is tracked by a separate version
	 * history column).
	 * 
	 * @usage _general_method_
	 */
	...
	public void validate(JetFormat format){
		...
		if (getType().isUnsupported()) {
			throw new IllegalArgumentException("Cannot create column with unsupported type "
				+ getType());
		}
		if (!format.isSupportedDataType(getType())) {
			throw new IllegalArgumentException("Database format " + format
				+ " does not support type " + getType());
		}
	...
	private static void writeCurrencyValue(ByteBuffer buffer, Object value) throws IOException{
		...
		try {
			...
			throw (IOException) new IOException("Currency value '" + inValue + "' out of range")
				.initCause(e);
		}
	}
	...
	private void writeNumericValue(ByteBuffer buffer, Object value) throws IOException{
		...
		try {
			...
			throw (IOException) new IOException("Numeric value '" + inValue + "' out of range")
				.initCause(e);
		}
	}
	...
	/**
	 * Returns {@code true} if the given text can be compressed using simple ASCII encoding,
		...
		for (Column col : columns) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("Creating new column def block\n"
					+ ByteUtil.toHexString(buffer, position,
						creator.getFormat().SIZE_COLUMN_DEF_BLOCK));
			}
		}
		for (Column col : columns) {
