...
public abstract class Cursor implements Iterable<Map<String, Object>> {
	...
	public static Cursor createIndexCursor(Table table, Index index, Object[] startRow,
		...
		return IndexCursor.createCursor(table, index, startRow, startInclusive, endRow,
			endInclusive);
	}
	 ...
	 * <p>
	 * Warning, this method <i>always</i> starts searching from the beginning of the Table (you
	 * cannot use it to find successive matches).
	 * 
	 * @param table
	 *            the table to search
	 * @param rowPattern
	...
	/**
	 * Convenience method for finding a specific row in a table which matches a given row "pattern".
	 * See {@link #findFirstRow(Column,Object)} for details on the pattern.
	 * <p>
	 ...
	 * "pattern". See {@link #findFirstRow(Map)} for details on the rowPattern.
	 * <p>
	 * Warning, this method <i>always</i> starts searching from the beginning of the Table (you
	 * cannot use it to find successive matches).
	 * 
	 ...
	 * @return the matching row or {@code null} if a match could not be found.
	 */
	public static Map<String, Object> findRow(Table table, Index index, Map<String, ?> rowPattern)
		throws IOException{
	...
	/**
	 * Sets a new ColumnMatcher. If {@code null}, resets to using the default matcher,
	 * {@link SimpleColumnMatcher#INSTANCE}.
	 */
	...
	/**
	 * Returns {@code true} if the cursor is currently positioned before the first row,
	...
	/**
	 * Calls <code>beforeFirst</code> on this cursor and returns a modifiable Iterator which will
	...
	/**
	 * Calls <code>beforeFirst</code> on this table and returns a modifiable Iterator which will
	...
	/**
	 * Returns an Iterable whose iterator() method returns the result of a call to
	 * {@link #columnMatchIterable(Column,Object)}
	 * 
	 * @throws IllegalStateException
	 *             if an IOException is thrown by one of the operations, the actual exception will
	 *             be contained within
	 ...
	 * Iterator follows the same restrictions as a call to <code>getNextRow</code>. See
	...
	/**
	 * Returns an Iterable whose iterator() method returns the result of a call to
	 * {@link #columnMatchIterator(Collection,Column,Object)}
	 * 
	 * @throws IllegalStateException
	 *             if an IOException is thrown by one of the operations, the actual exception will
	 *             be contained within
	 */
	public Iterable<Map<String, Object>> columnMatchIterable(final Collection<String> columnNames,
		final Column columnPattern, final Object valuePattern){
		return new Iterable<Map<String, Object>>() {
			public Iterator<Map<String, Object>> iterator(){
				return Cursor.this.columnMatchIterator(columnNames, columnPattern, valuePattern);
			}
		};
	}
	 ...
	 * <code>getNextRow</code>. See {@link #findFirstRow(Column,Object)} for details on the
	 * columnPattern.
	 * 
	 * @throws IllegalStateException
	 *             if an IOException is thrown by one of the operations, the actual exception will
	...
	/**
	 * Returns an Iterable whose iterator() method returns the result of a call to
	 * {@link #rowMatchIterator(Map)}
	 * 
	 * @throws IllegalStateException
	 *             if an IOException is thrown by one of the operations, the actual exception will
	 *             be contained within
	 */
	public Iterable<Map<String, Object>> rowMatchIterable(Map<String, ?> rowPattern){
	 ...
	 *             be contained within
	 */
	public Iterator<Map<String, Object>> rowMatchIterator(Map<String, ?> rowPattern){
		return rowMatchIterator(null, rowPattern);
	}
	
	/**
	 * Returns an Iterable whose iterator() method returns the result of a call to
	 * {@link #rowMatchIterator(Collection,Map)}
	 * 
	 * @throws IllegalStateException
	 *             if an IOException is thrown by one of the operations, the actual exception will
	 *             be contained within
	 */
	public Iterable<Map<String, Object>> rowMatchIterable(final Collection<String> columnNames,
		final Map<String, ?> rowPattern){
		return new Iterable<Map<String, Object>>() {
			public Iterator<Map<String, Object>> iterator(){
				return Cursor.this.rowMatchIterator(columnNames, rowPattern);
			}
		};
	}
	 ...
	 * <code>getNextRow</code>. See {@link #findFirstRow(Map)} for details on the rowPattern.
	 * 
	 * @throws IllegalStateException
	 *             if an IOException is thrown by one of the operations, the actual exception will
	 *             be contained within
	 ...
	 */
	@Deprecated
	public boolean findRow(Map<String, ?> rowPattern) throws IOException{
	...
	/**
	 * Moves to the next row (as defined by the cursor) where the given columns have the given
	 * values. This may be more efficient on some cursors than others. If a match is not found (or
	 * an exception is thrown), the cursor is restored to its previous state.
	 * 
	 * @param rowPattern
	...
	public boolean currentRowMatches(Map<String, ?> rowPattern) throws IOException{
		...
		for (Map.Entry<String, Object> e : row.entrySet()) {
			...
			if (!_columnMatcher.matches(getTable(), columnName, rowPattern.get(columnName),
				e.getValue())) {
				return false;
			}
		}
	 ...
	 */
	protected abstract class BaseIterator implements Iterator<Map<String, Object>> {
	...
	private final class ColumnMatchIterator extends BaseIterator {
		...
		private ColumnMatchIterator(Collection<String> columnNames, Column columnPattern,
			Object valuePattern){
			super(columnNames);
	...
	private final class RowMatchIterator extends BaseIterator {
		...
		private RowMatchIterator(Collection<String> columnNames, Map<String, ?> rowPattern){
			super(columnNames);
