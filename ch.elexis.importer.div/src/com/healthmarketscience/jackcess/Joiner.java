...
public class Joiner {
	...
	/**
	 * Creates a new Joiner based on the foreign-key relationship between the given
	 * "from"" table and the given "to"" table.
	 * 
	 * @param fromTable
	 ...
	 *             if there is no relationship between the given tables
	 */
	...
	/**
	 * Creates a new Joiner based on the given index which backs a foreign-key relationship. The
	 * table of the given index will be the "from" table and the table on the other end of the
	 * relationship will be the "to" table.
	 * 
	 * @param fromIndex
	 *            the index backing one side of a foreign-key relationship
	 */
	...
	/**
	 * Creates a new Joiner that is the reverse of this Joiner (the "from" and "to" tables are
	 * swapped).
	 */
	...
	/**
	 * Returns the first row in the "to" table based on the given columns in the "from" table if
	 * any, {@code null} if there is no matching row.
	 * 
	 * @param fromRow
	 *            row from the "from" table (which must include the relevant columns for this join
	 *            relationship)
	 */
	...
	/**
	 * Returns selected columns from the first row in the "to" table based on the given columns in
	 * the "from" table if any, {@code null} if there is no matching row.
	 * 
	 * @param fromRow
	 *            row from the "from" table (which must include the relevant columns for this join
	 *            relationship)
	 * @param columnNames
	 *            desired columns in the from table row
	 */
	public Map<String, Object> findFirstRow(Map<String, ?> fromRow, Collection<String> columnNames)
		throws IOException{
		toEntryValues(fromRow);
		return ((_toCursor.findFirstRowByEntry(_entryValues) ? _toCursor.getCurrentRow(columnNames)
				: null));
	}
	
	/**
	 * Returns an Iterator over all the rows in the "to" table based on the given columns in the
	 * "from" table.
	 * 
	 * @param fromRow
	 *            row from the "from" table (which must include the relevant columns for this join
	...
	/**
	 * Returns an Iterator with the selected columns over all the rows in the "to" table based on
	 * the given columns in the "from" table.
	 * 
	 * @param fromRow
	 *            row from the "from" table (which must include the relevant columns for this join
	 *            relationship)
	 ...
	 *             be contained within
	 */
	public Iterable<Map<String, Object>> findRowsIterable(final Map<String, ?> fromRow,
