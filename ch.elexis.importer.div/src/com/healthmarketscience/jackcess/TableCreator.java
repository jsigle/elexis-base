...
class TableCreator {
	...
	public TableCreator(Database database, String name, List<Column> columns,
		...
		_name = name;
		_columns = columns;
		_indexes = ((indexes != null) ? indexes : Collections.<IndexBuilder> emptyList());
	}
	...
	private void validate(){
		...
		Database.validateIdentifierName(_name, getFormat().MAX_TABLE_NAME_LENGTH, "table");
		...
		if (autoCols.size() > 1) {
			...
			for (Column c : autoCols) {
				if (!c.getType().isMultipleAutoNumberAllowed() && !autoTypes.add(c.getType())) {
		...
		if (hasIndexes()) {
			...
			for (IndexBuilder index : _indexes) {
				...
				if (index.isPrimaryKey()) {
					if (foundPk) {
						throw new IllegalArgumentException("found second primary key index: "
							+ index.getName());
					}
					foundPk = true;
				}
