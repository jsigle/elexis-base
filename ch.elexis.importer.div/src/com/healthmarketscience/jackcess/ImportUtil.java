...
public class ImportUtil {
	 ...
	 *            table with unique name
	 * @param header
	 *            if {@code false} the first line is not a header row, only valid if
	 *            useExistingTable is {@code true}
	 * @return the name of the imported table
	 * 
	 ...
	 */
	public static String importReader(BufferedReader in, Database db, String name, String delim,
		char quote, ImportFilter filter, boolean useExistingTable) throws IOException{
	 ...
	 */
	public static String importReader(BufferedReader in, Database db, String name, String delim,
		char quote, ImportFilter filter, boolean useExistingTable, boolean header)
		throws IOException{
			...
			if (!useExistingTable || ((table = db.getTable(name)) == null)) {
				...
				for (int i = 0; i < columnNames.length; i++) {
					columns.add(new ColumnBuilder((String) columnNames[i], DataType.TEXT)
						.escapeName().setLength((short) DataType.TEXT.getMaxSize()).toColumn());
				}
	...
	/**
	 * Splits the given line using the given delimiter pattern and quote character. May read
	 * additional lines for quotes spanning newlines.
	 */
	private static Object[] splitLine(String line, Pattern delim, char quote, BufferedReader in,
	...
	public static class Builder {
		...
		public String importReader(BufferedReader reader) throws IOException{
			return ImportUtil.importReader(reader, _db, _tableName, _delim, _quote, _filter,
				_useExistingTable, _header);
		}
	}
